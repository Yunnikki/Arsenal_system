...
if ((currentWeapon _unit == "HLA_APF_LMG") and (isNil {_unit getVariable "WBK_CombineType"})) exitWith {hint "Только Подавитель может использовать это";};
if !(_unit getVariable "WBK_CombineType" == " apf_") exitWith {hint "Только Подавитель может использовать это";};
